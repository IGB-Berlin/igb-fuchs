...
test('different timezone', async ({ page }) => {
  ...
  else await page.getByTestId('logEndTime').getByRole('textbox').fill('2024-01-02T05:05')
  await expect(page.getByTestId('logEndTime').getByRole('textbox')).toHaveValue('2024-01-02T05:05')
  ...
  if (isWebkit) {  // same as above
    await page.getByTestId('locStartTime').getByRole('button').click()
    await expect(page.getByRole('button', { name: 'Use current time' })).toBeVisible()
    await page.getByRole('button', { name: 'Use current time' }).click()
  }
  else await page.getByTestId('locStartTime').getByRole('textbox').fill('2024-01-02T03:03')
  await expect(page.getByTestId('locStartTime').getByRole('textbox')).toHaveValue('2024-01-02T03:03')
