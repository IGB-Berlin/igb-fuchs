...
rm -rf dist-tsc.tmp/ dist-inst.tmp/ dist-cover/ .nyc_output coverage

# use tsc to compile .ts/x to .js
( cd src && npx tsc --removeComments false --outDir ../dist-tsc.tmp/ )
# then use nyc to instrument the code coverage collection
npx nyc instrument --compact=false --all dist-tsc.tmp/ dist-inst.tmp/

# build a directory structure that mirrors the original,
# with all the required files, so Parcel can build normally
cp -r src/index.html src/styles.scss src/manifest.json src/images dist-inst.tmp/
cp src/types/igb-fuchs.schema.json dist-inst.tmp/types/

# use parcel to bundle; builds to dist-cover/
npx parcel build --target cover dist-inst.tmp/index.html
cp -v dist-inst.tmp/types/igb-fuchs.schema.json dist-cover/
...
WEB_SERVER_PATH=dist-cover npx playwright test "${test_args[@]}"
